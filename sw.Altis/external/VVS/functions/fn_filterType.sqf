...
_vehicleList = switch(_filter) do
{
	...
	case "Наземная_техника_ВАР": {VVS_R_Car};
	case "Воздушная_техника_ВАР": {VVS_R_Air};
	case "Наемники": { VVS_Air_Mercenary};
	case "Наземная_техника_Наемники": {VVS_Car_Mercenary};
	case "Ремонт": {VVS_Air_Mercenary};
	case "Транспорт": {VVS_Car};
	case "Броня": {VVS_HEVY};
	case "БПА": {VVS_pre_Autonomous};
	case "Тяж авиация": {VVS_CIS};
